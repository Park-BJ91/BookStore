...
/* 	 작가 정보 수정 버튼의 경우는 먼저 anmeCk와 introCk 변수를 선언하여 기본적으로 false 값으로 초기화를 합니다.
	 그리고 작가 이름, 작가 소개 <input> 태그가 공란이 아닌지 if문을 통해 체크하여 공란일 경우 Ck값을 false를 대입 및 <span> 태그 노출시키고 
	 공란이 아닐 경우 Ck값을 true로 대입 및 <span> 태그를 숨깁니다. 
	 마지막 최종적으로 Ck값이 모드 true일 때 작가 정보 데이터가 담긴 <form>이 서버에 전송되도록 하였습니다.  */
